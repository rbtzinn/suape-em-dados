...
4. Crie os arquivos `Code.gs` e `Importacao.gs` no Apps Script e publique como aplicativo da Web executado pelo proprietário da planilha.
...
Uma conta existente, ativa e autenticada no Firebase entra como `VIEWER`. As listas da Vercel servem apenas para elevar usuários a `ANALYST` ou `ADMIN`; não existe tela de cadastro no portal. Para impedir também o cadastro pela API pública, desative a criação de contas por usuários finais nas configurações de Authentication/Identity Platform e mantenha a criação manual pela equipe gestora.

Se o Firebase responder `400`, o formulário agora mostra a causa útil. As mais comuns são provedor **E-mail/senha** desativado, chave pública incorreta ou credencial inválida. A rota correta termina em `accounts:signInWithPassword`; uma chamada a `/v1/accounts` sem essa ação retorna `404`.
